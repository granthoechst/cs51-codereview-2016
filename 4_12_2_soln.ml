...
object
  ...
  method! scale (k: float) : unit =
    ...
    let (x_mid_old, y_mid_old) = ((x1 +. x2) /. 2., (y1 +. y2)  /. 2.) in
    (* first scale like normal *)
    let _ = super#scale k in
    let ((x1,y1),(x2,y2)) = super#bounding_box in
    let (x_mid, y_mid) = ((x1 +. x2) /. 2., (y1 +. y2)  /. 2.) in
    (* find how off we are *)
    let (diff_x, diff_y) = (x_mid_old -. x_mid, y_mid_old -. y_mid) in
    super#translate (diff_x, diff_y)
...
object
  (* We don't need to say "inherit ... as super" if we never plan on using the
   * variable "super". In fact, you'd get a warning. *)
